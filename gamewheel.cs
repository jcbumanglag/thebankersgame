...
namespace bankers_game_proto
{
    class gamewheel
    {
        int finalsec;  //ito ung resulta ng napili ng wheel
        public string result; // resulta ng nakuha sa wheel

        // eto ung parte ng wheel
        Sector[] wheel;

        // geometry ng mga variables
        Point center; //gitna ng wheel
        int radius;   // radius ng wheel
        float sectorangle; //angle ng kada sector
        Rectangle boundrect;
            ...
            switch (this.state)
            {
                ...
                case STATE_SLOWING_DOWN:
                    ...
                    if (rotateAngle <= 0)
                    {
                        ...
                        this.finalsec = this.wheel.Length - (int)(((wheel[0].offsetAngle + 90) % 360) / this.sectorangle) - 1;
                        this.result = wheel[finalsec].value;
                        wheel[finalsec].fillBrush.Color = Color.Black;
                ...
                case STATE_HIGHLIGHTING:
                    ...
                    Color old = wheel[finalsec].fillBrush.Color;
                    ...
                    if (highlight == true)
                    {
                        wheel[finalsec].fillBrush.Color = Color.FromArgb(old.R + 15, old.R + 15, 0);
                    }
                    else
                    {
                        wheel[finalsec].fillBrush.Color = Color.FromArgb(old.R - 15, old.R - 15, 0);
